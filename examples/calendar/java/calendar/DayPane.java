...
//			double lY = mouseEvent.getScreenY()
//					- NodeUtil.screenY(DayPane.this);
...
//			}
//
//			// - calculate the number of pixels from onscreen nodeY (layoutY) to
//			// onscreen mouseY
//			double lHeight = mouseEvent.getScreenY()
//					- NodeUtil.screenY(resizeRectangle);
//			if (lHeight < 5) {
//				lHeight = 5;
//			}
...
//
//			// must have dragged (otherwise it is considered an "unselect all"
//			// action)
//			if (dragged == false) {
//				return;
//			}
...
//			Calendar lStartCalendar = setTimeTo0000((Calendar) DayPane.this.calendarObjectProperty
//					.get().clone());
//			lStartCalendar
//					.add(Calendar.MILLISECOND,
//							(int) (resizeRectangle.getY() * durationInMSPerPixelProperty
//									.get()));
//			setTimeToNearestMinutes(lStartCalendar, 5);
//
//			// calculate the new end date for the appointment (recalculating the
//			// duration)
//			Calendar lEndCalendar = (Calendar) lStartCalendar.clone();
//			lEndCalendar
//					.add(Calendar.MILLISECOND, (int) (resizeRectangle
//							.getHeight() * durationInMSPerPixelProperty.get()));
...
//		List<AbstractAppointmentPane> lPanes = new ArrayList<AgendaWeekSkin.AbstractAppointmentPane>(
...
//		double lRemainingWidthForRegularAppointments = dayContentWidthProperty
...
//			double lW = (dayContentWidthProperty.get() - (wholedayAppointmentPanes
//					.size() * wholedayAppointmentWidth))
//					* (1.0 / (((double) lClusteredDayAppointmentPanePane.clusterOwner.clusterTracks
//							.size())));
//			// all but the most right appointment get 50% extra width, so they
//			// underlap the next track
//			if (lClusteredDayAppointmentPanePane.clusterTrackIdx < lClusteredDayAppointmentPanePane.clusterOwner.clusterTracks
//					.size() - 1)
//				lW *= 1.75;
...
//		final List<AbstractClusteredDayAppointmentPane> lOldClusteredDayAppointmentPanes = new ArrayList<AbstractClusteredDayAppointmentPane>(
//				clusteredAppointmentPanes);
//		final List<WholedayAppointmentPane> lOldWholedayAppointmentPanes = new ArrayList<WholedayAppointmentPane>(
...
//					WholedayAppointmentPane lAppointmentPane = new WholedayAppointmentPane(
//							lAppointment, this);
...
//						&& lOverlaps == false; i++) {
//					lOverlaps = checkIfTheAppointmentOverlapsAnAppointmentAlreadyInThisTrack(
...
//		// TODO: animation? we could move the old appointments to the equivalent
//		// positions on the drag pane, then animate them to their new positions,
//		// remove the old, and insert the new ones.
//		// however, this needs to be cross-days, so it cannot be done here (this
